...
async def register(request: RegisterRequest, db: Session = Depends(get_db)):
    ...
    refresh_token = generate_token(user_id, request.email, token_type="refresh")
...
async def login(request: LoginRequest, db: Session = Depends(get_db)):
    ...
    # Generate tokens
    access_token = generate_token(user.user_id, user.email, token_type="access")
    refresh_token = generate_token(user.user_id, user.email, token_type="refresh")
...
async def refresh(request: RefreshTokenRequest):
    ...
    # Generate new tokens
    access_token = generate_token(token_data.user_id, token_data.email, token_type="access")
    refresh_token = generate_token(token_data.user_id, token_data.email, token_type="refresh")
...
async def get_me(current_user: UserContext = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    """
    user = db.query(UserProfile).filter(UserProfile.user_id == current_user.user_id).first()
    if not user:
        raise HTTPException(
