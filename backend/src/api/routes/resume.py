...
# Helper function to get user profile from authenticated user


def get_user_profile(user: UserContext, db: Session) -> UserProfile:
    """Get UserProfile for authenticated user."""
...
# Resume file upload/parsing configuration
...
async def parse_resume_background(
    ...
    try:
        ...
        resume = db_session.query(ParsedResume).filter(
            ParsedResume.id == resume_id).first()
        if resume:
            ...
            db_session.commit()
            logger.info(
                f"Resume {resume_id} parsed successfully (confidence: {parsed_data['parsing_confidence']:.2f})")
...
async def upload_resume(
    ...
    # Get user profile
    user_profile = get_user_profile(user, db)

    # Create ParsedResume record
    parsed_resume = ParsedResume(
        user_profile_id=user_profile.id,  # FK to user_profiles.id
...
async def get_resume_status(
    ...
    resume = db.query(ParsedResume).filter(
        ...
        ParsedResume.user_profile_id == user_profile.id,
    ).first()
