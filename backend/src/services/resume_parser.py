...
class ResumeParser:
    ...
    def _calculate_confidence(self, text: str) -> float:
        ...
        matches = sum(1 for indicator in indicators if re.search(indicator, text, re.IGNORECASE))
    ...
    def _extract_achievements(self, text: str) -> list:
        ...
        metric_patterns = [
            ...
            r'(?:increased|reduced|improved|grew|saved|generated)\s+(?:by\s+)?\d+',  # Actions with numbers
        ]
        
        for bullet in bullets:
            bullet = bullet.strip()
            # Check if bullet contains metrics
            has_metric = any(re.search(pattern, bullet, re.IGNORECASE) for pattern in metric_patterns)
    ...
    def _extract_companies(self, text: str) -> list:
        ...
        pattern = r'([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s+(?:' + '|'.join(company_keywords) + r')'
    ...
    def _calculate_confidence(self, text: str) -> float:
        ...
        matches = sum(1 for indicator in section_indicators if re.search(indicator, text, re.IGNORECASE))
