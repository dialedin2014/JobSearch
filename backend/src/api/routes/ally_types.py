...
class CreateAllyTypeRequest(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=100, description="Ally type name (e.g., 'AI Researchers')")
    keywords: List[str] = Field(..., description="List of keywords for matching (e.g., ['machine learning', 'NLP'])")
    criteria: str = Field(..., min_length=1, description="User-defined criteria for this ally type")
    search_parameters: Optional[Dict[str, Any]] = Field(default=None, description="Platform-specific search parameters")
...
async def delete_ally_type(
    ...
    logger.info(f"Soft deleted ally type {ally_type_id} for user {user.user_id}")
...
async def export_ally_types(
    ...
    logger.info(f"Exported {len(ally_types)} ally types for user {user.user_id}")
...
async def import_ally_types(
    ...
    logger.info(f"Imported {imported_count} ally types for user {user.user_id} (skipped {skipped_count})")
    
    return {
        "message": f"Successfully imported {imported_count} ally types",
...
async def get_analysis_results(
    ...
    if min_confidence:
        query = query.filter(DeducedAllyType.confidence_score >= min_confidence)
